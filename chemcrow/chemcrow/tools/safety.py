...
class MoleculeSafety:
    ...
    def _fetch_pubchem_data(self, cas_number):
        ...
        Note: Caching disabled for benchmark accuracy - each call will fetch fresh data.
        """
        # For benchmarking, we want fresh data each time to measure true API latency
        # Comment out the cache check for accurate timing measurements
        # if cas_number not in self.pubchem_data:
        try:
            print(f"🌐 Fetching PubChem data for CAS: {cas_number}")
            ...
            r = requests.get(url)
            data = r.json()
            # Store in cache but always fetch fresh for timing accuracy
            self.pubchem_data[cas_number] = data
            return data
